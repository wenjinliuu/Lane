...
- 公开配置只写一条 Lane 自托管的空节点资源，标签固定为 `Lane 节点订阅占位`，默认 `enabled=false`，不提供任何可用代理，也不主动联网更新。更新间隔预设为 86400 秒，用户替换 URL 并启用后即可正常自动更新。
- 用户可在 QX 节点资源页面另行添加订阅；若直接编辑配置，则替换占位 URL 并显式改为 `enabled=true`。
- 配置顶部和 `[server_remote]` 段内都必须明确标出 `设置 → 节点 → 节点资源` 路径，以及直接编辑时需要替换 URL、启用资源、多订阅逐行添加的步骤。
- 校验器锁定 12 个模块的完整顺序、六个空模块、资源 URL、标签、禁用状态和空文件内容，防止以后把示例节点、第三方订阅、重写或 MitM 参数误当作默认配置发布。
