...
| 客户端 | 问题 | 修复 |
|---|---|---|
| **Stash** | `include-all + proxies` 会使来源关系不明确；后续重新核对官方语法与实际配置，确认显式 `use + proxies` 可混合 | 改为单层 `Manual`：`proxies` 放五个地区 Auto，`use` 按名称引用 `Subscription1` 并直接展开全部节点；多订阅名称必须在 `proxy-providers` 与 `Manual.use` 完全一致 |
