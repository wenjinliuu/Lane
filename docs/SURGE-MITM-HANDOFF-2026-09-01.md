...
| 客户端 | 问题 | 修复 |
|---|---|---|
| **Stash** | 显式 `use` 能混合地区组与节点，但要求用户在每次新增代理集时同步维护名称 | 改为单层 `Manual`：`proxies` 放五个地区 Auto，`include-all: true` 自动展开全部节点；代理集名称可自定义，新增订阅或覆写无需修改 `Manual` |
| **Quantumult X** | 中文占位符和注释订阅模板都会导致完整配置导入失败；真机还会依次检查 `[server_remote]` 与 `[server_local]` | 保留空的 `[server_local]`；`[server_remote]` 引用 Lane 自托管、内容为空、默认禁用且不自动更新的占位资源；导入后在 App 内添加真实节点资源，或替换该行 URL 并启用；`excluded_routes` 仍为纯 IPv4 |
