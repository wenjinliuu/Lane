...
CONFIG_FILENAMES = {
    ...
    "clash-verge-rev": "Lane_clash_verge_rev.yaml",
}
CONFIG_COMPAT_FILENAMES = {
    "loon": ("Lane_loon.conf",),
}
TARGETS = tuple(CONFIG_FILENAMES)
MIHOMO_TARGETS = ("flclash", "clash-verge-rev")
RULES_DIR = "rules"
STASH_BEHAVIOR_ORDER = ("domain", "ipcidr", "classical")
GENERATED_HEADER = "# Generated by Lane. Do not edit generated rule files.\n"
LOON_CN_REGION_RULE_ID = "cn-region"
LOON_CN_REGION_RULE_TITLE = "CN Region"
LOON_CN_REGION_RULE_POLICY = "DIRECT"
LOON_CN_REGION_RULE_CONTENT = GENERATED_HEADER + "GEOIP,CN\n"
...
QX_SUBSCRIPTION_GUIDANCE_MARKERS = (
    "Lane 不提供节点；[server_remote] 保持为空，不内置虚拟地址或订阅占位",
    "设置 → 节点 → 节点资源",
    "在应用内添加",
)
...
# home screen, so the profile references it instead of defining Lane's Proxy group.
...
QX_LOCAL_PROFILE_NOTICE = (
    "# 节点订阅入口：导入后进入「设置 → 节点 → 节点资源」，在应用内添加。\n"
    "# Lane 不提供节点；[server_remote] 保持为空，不内置虚拟地址或订阅占位。\n"
    "# QX 完整配置要求的未使用模块只保留空段；不会启用重写、脚本、HTTP 后端或 MitM。\n"
    "# 建议复制/保存为本地配置；换用新版完整配置前，请备份节点资源与个人修改。\n"
)
SERVICE_GROUP_NOTICE = (
    ...
    "# 上述两组默认均为 Proxy；分流只控制网络路径，不保证入金或交易结果。\n"
...
CLIENT_NOTES = {
    "stash": (
        ...
        "DOMAIN-REGEX and other non-specialized rules remain classical. A dedicated "
        "我的节点 group includes every provider, while Proxy keeps regional groups visible."
    ),
    "loon": (
        ...
        "compatibility aliases retained for existing installations. GEOIP,CN "
        "is delivered by the final cn-region remote rule so local rules do not "
        "take priority over plugin rules."
    ),
    "shadowrocket": (
        ...
        "generated Proxy group; icons are omitted because Shadowrocket has no "
        "policy-group icon parameter."
    ),
    "surge": (
        "DOMAIN-REGEX is unsupported and omitted. Region automation uses native "
        "Smart groups over the visible 我的节点 pool; visible groups carry icon-url."
    ),
    "qx": (
        ...
        "The official complete-profile section skeleton is preserved. server_remote "
        "and all unused rewrite, script, HTTP backend, and MitM sections remain empty."
    ),
    "egern": (
    ...
    "clash-verge-rev": (
        "One native Mihomo YAML profile for Clash Verge Rev on Windows, macOS and "
        "Linux. A local proxy-provider keeps the private node subscription separate "
        "from Lane's remote classical rule-providers. All three operating systems "
        "use the same profile schema; Proxy exposes 我的节点 before the five regional "
        "Auto groups."
    ),
}
...
def render_rule(
    rule: Rule, target: str, no_resolve: bool = False, policy: str = BASE_GROUP_NAME
) -> str | None:
    kinds = {
        "full": "DOMAIN",
    ...
    if rule.kind == "regexp" and target not in {"stash", *MIHOMO_TARGETS}:
        return None
...
def _shadowrocket_policy(policy: str) -> str:
    """Map Lane's shared Proxy policy to Shadowrocket's built-in PROXY."""

    return SHADOWROCKET_MANUAL_POLICY if policy == BASE_GROUP_NAME else policy
...
def _stash_config(
    ...
    # Keep dynamic provider nodes in their own group. On-device Stash testing
    # shows that mixing ``proxies`` with ``use`` or ``include-all`` in one group
    # can hide either the fixed region groups or the raw nodes.
    groups.append(
        _with_icon(
            {
    ...
    body = body.replace(
        ...
        f"  # {NODE_GROUP_NAME} 与地区组使用 include-all，会自动纳入所有代理集，无需维护订阅名称。\n"
        "  # 使用覆写新增代理集时只需新增 proxy-providers 项，不需要修改策略组。\n"
        + optional_subscription
        + f"\n# {NODE_GROUP_NAME} 排在 Proxy 前；Proxy 通过它进入全部单节点。\n"
...
def _loon_config(
    ...
    lines = [
        ...
        "[Remote Filter]",
        f'{NODE_GROUP_NAME} = NameRegex, FilterKey = "{filters["manual"]}"',
    ]
    for region in policies["regions"]:
        code = region["code"]
        region_filter = filters["regions"][region["name"]]
    ...
    lines.append(
        _loon_group(
            BASE_GROUP_NAME,
            "select",
            [*_region_auto_names(policies), NODE_GROUP_NAME],
            benchmark,
            _icon(icons, BASE_GROUP_NAME),
        )
    )
    for service in policies["service_groups"]:
        lines.append(
    ...
    cn_region_url = (
        f"{raw_base}/dist/loon/{RULES_DIR}/"
        f"{rule_filename('loon', LOON_CN_REGION_RULE_ID)}"
    )
    lines.append(
        f"{cn_region_url}, policy = {LOON_CN_REGION_RULE_POLICY}, "
        f"tag = {LOON_CN_REGION_RULE_TITLE}, enabled = true"
    )
    lines.extend(
        ["", "# Local fallback rule. GEOIP,CN is the final remote rule above.",
         "[Rule]", "FINAL,Final"]
    )
    return "\n".join(lines) + "\n"
...
def shadowrocket_options(policies: dict[str, Any]) -> list[str]:
    """Service options for Shadowrocket, mapping Lane Proxy to built-in PROXY."""

    return [_shadowrocket_policy(option) for option in policies["service_options"]]
...
def _shadowrocket_config(
    ...
    for entry in policies["_compiled_rulesets"]:
        ...
        lines.append(f"RULE-SET,{url},{_shadowrocket_policy(entry.policy)}")
...
def _surge_config(
    ...
    lines.append(f"# 地区组从 {NODE_GROUP_NAME} 展开真实节点并正向筛选；不会与 Proxy 形成循环。")
    for region in policies["regions"]:
...
def _qx_config(
    ...
    lines = [
        ...
        "# [server_remote] 只保留空段；节点订阅由用户在 QX 应用内维护。", "",
    ...
    lines.extend([
        "", "# ==================== 节点订阅 ====================",
        "[server_remote]",
        "# [server_remote] 保持为空。导入 Lane 后，请进入 QX「设置 → 节点 → 节点资源」在应用内添加订阅。",
        "", "[filter_remote]",
    ...
    lines.extend([
        "", "[rewrite_remote]",
        "# Lane 不加载远程重写资源；保留空模块以满足完整配置导入检查。",
        "", "[server_local]",
        "# Lane 不内置本地节点；保留空模块以满足完整配置导入检查。",
        "", "[filter_local]", "geoip,cn,direct", "final,Final",
        "", "[rewrite_local]",
        "# Lane 不配置本地重写。",
        "", "[task_local]",
        "# Lane 不配置定时或事件脚本。",
        ...
        "# Lane 不配置本地 HTTP 后端。",
        "", "[mitm]",
        "# Lane 不配置证书、主机名或 HTTPS 解密；本模块保持为空。",
    ])
    return "\n".join(lines) + "\n"
...
def _egern_config(
    ...
    for region in policies["regions"]:
        for kind, name in (("auto_test", region["auto_name"]), ("select", region["manual_name"])):
            group = {
                "name": name, "policies": [NODE_GROUP_NAME], "flatten": True,
    ...
    body = _yaml(data).replace(
        "policy_groups:\n",
        f"# 只需在 {NODE_GROUP_NAME} 的 urls 填写订阅；Proxy 通过它进入全部单节点。\n"
        "policy_groups:\n",
        1,
    )
    body = body.replace(
        ...
        f"    - {SUBSCRIPTION_PLACEHOLDER}\n"
        f"    # 多订阅：取消下一行注释并填写；地区组会自动从 {NODE_GROUP_NAME} 展开。\n"
        f"    # - {SUBSCRIPTION_PLACEHOLDER}\n",
        1,
    )
...
def _mihomo_config(
    ...
    *,
    ...
    """Render a local profile using the shared native Mihomo schema."""

    if target not in MIHOMO_TARGETS:
        raise ValueError(f"Unknown Mihomo target: {target}")
    ...
    for entry in policies["_compiled_rulesets"]:
        providers[entry.id] = {
            ...
            "behavior": "classical",
            "format": "text",
            "url": f"{raw_base}/dist/{target}/{RULES_DIR}/{entry.id}.list",
    ...
    body = body.replace(
        "proxy-providers:\n",
        f"# {client_name}：在下面的 url 填写机场提供的 Clash / Mihomo 节点订阅；第一份默认启用。\n"
        "proxy-providers:\n",
        1,
    )
    second = {
...
def _flclash_config(
    ...
    """Render the Android FlClash profile with the native Mihomo schema."""

    return _mihomo_config(
        project, policies, icons, target="flclash", client_name="FlClash"
    )


def _clash_verge_rev_config(
    project: dict[str, Any], policies: dict[str, Any], icons: dict[str, Any]
) -> str:
    """Render one portable Clash Verge Rev profile for all desktop platforms."""
    ...
    return _mihomo_config(
        ...
        client_name="Clash Verge Rev（Windows / macOS / Linux 共用）",
    )
...
def render_all(
    ...
    for target in TARGETS:
        for ruleset in rulesets:
            ...
            if target == "egern":
                ...
                for rule in ruleset.rules:
                    ...
                    payload_no_resolve = (
                        ruleset.no_resolve and target not in MIHOMO_TARGETS
                    )
                    line = render_rule(
        ...
        if target == "loon":
            for filename in rule_filenames(target, LOON_CN_REGION_RULE_ID):
                path = dist / target / RULES_DIR / filename
                expected[target].add(path)
                write_if_changed(path, LOON_CN_REGION_RULE_CONTENT)
    ...
    renderers = {
        ...
        "clash-verge-rev": _clash_verge_rev_config,
    }
    for target in TARGETS:
