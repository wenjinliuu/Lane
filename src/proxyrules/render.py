...
SUBSCRIPTION_URL = "https://example.com/replace-with-your-subscription"
LOCAL_PROFILE_NOTICE = (
    "# 使用前将示例链接替换为本客户端支持的节点订阅；示例地址不提供节点。\n"
    "# 建议复制/保存为本地配置，不跟随远程整份更新；节点与远程规则仍独立更新。\n"
    "# 换用新版完整配置前，请备份并重新填入订阅地址及个人修改；勿上传私人订阅。\n"
)
EGERN_RULE_FIELDS = {
...
def _stash_config(
    ...
    body = body.replace(
        "proxy-providers:\n",
        "# 在下面的 url 填写 Stash / Clash 格式节点订阅。\nproxy-providers:\n",
        1,
    )
    body = body.replace(
        "proxy-groups:\n",
...
def _surge_config(
    ...
    filters = build_filters(policies)
    lines = [
        PROFILE_HEADER.rstrip(), LOCAL_PROFILE_NOTICE.rstrip(), "",
        "[General]", "loglevel = notify", "ipv6 = false",
        "allow-wifi-access = false", "dns-server = system",
        f"proxy-test-url = {benchmark['url']}",
        f"test-timeout = {benchmark['timeout']}", "", "[Proxy]", "",
        "[Proxy Group]",
        "# 只需替换 Manual 的 policy-path；必须使用 Surge 支持的节点订阅格式。",
        f"Manual = select,policy-path={SUBSCRIPTION_URL},"
        f"update-interval={updates['node_interval']},include-all-proxies=true",
    ]
...
def _qx_config(
    ...
    for region in policies["regions"]:
        ...
        lines.append(f"static = {region['manual_name']}, server-tag-regex={expression}"
                     f"{icon(region['name'])}")
    lines.extend([
        "", "[server_remote]", "# 替换下面的链接为 Quantumult X 格式节点订阅。",
        f"{SUBSCRIPTION_URL}, tag=Subscription, update-interval={updates['node_interval']}, enabled=true",
...
def _egern_config(
    ...
    groups = [{"select": _with_icon({
        "name": "Manual", "urls": [SUBSCRIPTION_URL],
    ...
    body = _yaml(data).replace(
        "policy_groups:\n", "# 在 Manual 的 urls 中填写 Egern 支持的节点订阅。\npolicy_groups:\n", 1
    )
    return PROFILE_HEADER + LOCAL_PROFILE_NOTICE + body
