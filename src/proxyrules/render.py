...
RULES_FULL_DIR = "rules-full"
RULES_PROFILE_DIR = "rules-profile"
...
# TUN bypass. Loon's official example excludes the RFC private and special-use
# prefixes at the interface level instead of relying on the lan ruleset alone;
# the other clients take the same list under their own key. The two layers do
# different jobs: these routes never enter the tunnel, while lan still answers
# for anything that does. 198.18.0.0/15 is deliberately absent — every supported
# client uses it as its fake-IP range, and bypassing it here would stop fake-IP
# traffic from ever reaching the rule engine.
PRIVATE_EXCLUDED_ROUTES = (
    "10.0.0.0/8",
    "100.64.0.0/10",
    "127.0.0.0/8",
    "169.254.0.0/16",
...
EXCLUDED_ROUTES = PRIVATE_EXCLUDED_ROUTES + MULTICAST_EXCLUDED_ROUTES
# Loon has no exclude-simple-hostnames switch, so name the local suffixes it can
# skip. A bare single-label host still falls through on Loon and QX.
LOON_SKIP_PROXY = ("localhost", "*.local", "*.lan", "*.home.arpa")
# Loon's [Remote Filter] entries take the subscriptions they read as positional
# arguments -- `HK = NameRegex,Subs,FilterKey = *HK` in the official example.
# Omitting them leaves every filter, and therefore every strategy group, empty.
LOON_SUBSCRIPTION_SOURCE = "Subscription1"
REAL_IP_DOMAINS = (
    "*.stun.*.*",
    "*.stun.*.*.*",
    "*.stun.*.*.*.*",
...
# Applications that hardcode a public resolver bypass the client's own DNS and
# therefore its routing. Hijacking the common ones keeps every lookup on the
# same path; a client-specific wildcard is deliberately not used.
HIJACK_DNS_SERVERS = (
    "1.1.1.1:53",
    "1.0.0.1:53",
    "8.8.8.8:53",
...
CLIENT_NOTES = {
    ...
    "loon": (
        ...
        "Remote filters name their subscription sources explicitly. "
        "Single-label hostnames have no native switch; skip-proxy covers the "
        "local suffixes only."
    ),
    "shadowrocket": (
        "DOMAIN-REGEX is omitted for compatibility. The lan single-label "
        "hostname regex is replaced by the native exclude-simple-hostnames."
    ),
    "surge": (
        "DOMAIN-REGEX is unsupported and omitted; the lan single-label hostname "
        "regex is replaced by the native exclude-simple-hostnames. Native "
        "url-test groups allow temporary manual override; icons are omitted for "
        "iOS compatibility."
    ),
    "qx": (
        ...
        "matching priorities. Single-label hostnames have no native switch."
    ),
    "egern": (
...
def _rule_url(
    ...
    target: str,
    rule_id: str,
    directory: str = RULES_PROFILE_DIR,
) -> str:
...
def _stash_config(
            ...
            _with_icon(
                {
                    ...
                    # Without an explicit url the group falls back to the
                    # client's own default, which is the one place the six
                    # profiles would benchmark against different targets.
                    "url": benchmark["url"],
    ...
    for entry in policies["_compiled_rulesets"]:
        providers[entry.id] = {
            ...
            "behavior": "classical",
            "format": "text",
            "url": f"{raw_base}/dist/stash/{RULES_PROFILE_DIR}/{entry.id}.list",
...
def _loon_config(
    ...
    lines = [
        ...
        f"skip-proxy = {', '.join(LOON_SKIP_PROXY)}",
        f"bypass-tun = {', '.join(EXCLUDED_ROUTES)}",
        ...
        "# 新增订阅后，还要把它的别名补进下方每一条 Remote Filter。",
        f"Subscription1 = {SUBSCRIPTION_PLACEHOLDER}",
        f"# Subscription2 = {SUBSCRIPTION_PLACEHOLDER}",
        "",
        "# Node and region filters. Each filter names the subscriptions it reads,",
        "# which is what Loon's own example does; the alias list is not optional.",
        "# 多订阅：在每行 NameRegex 之后按顺序补上别名，例如",
        '#   US Nodes = NameRegex,Subscription1,Subscription2,FilterKey = "..."',
        "[Remote Filter]",
        f'Manual Nodes = NameRegex,{LOON_SUBSCRIPTION_SOURCE},FilterKey = "{filters["manual"]}"',
    ]
    for region in policies["regions"]:
        code = region["code"]
        region_filter = filters["regions"][region["name"]]
        lines.append(
            f'{code} Nodes = NameRegex,{LOON_SUBSCRIPTION_SOURCE},'
            f'FilterKey = "{region_filter}"'
        )
    ...
    for entry in policies["_compiled_rulesets"]:
        url = f"{raw_base}/dist/loon/{RULES_PROFILE_DIR}/{entry.id}.list"
        lines.append(
...
def _shadowrocket_config(
    ...
    lines = [
        ...
        "[General]",
        "ipv6 = false",
        # Native replacement for the lan single-label hostname regex, which this
        # client cannot express as a rule.
        "exclude-simple-hostnames = true",
        f"tun-excluded-routes = {', '.join(EXCLUDED_ROUTES)}",
    ...
    for entry in policies["_compiled_rulesets"]:
        url = f"{raw_base}/dist/shadowrocket/{RULES_PROFILE_DIR}/{entry.id}.list"
...
def _qx_config(
    ...
    lines = [
        ...
        f"excluded_routes = {', '.join(EXCLUDED_ROUTES)}",
...
def _egern_config(
    ...
    data = {
        ...
        "vif_excluded_routes": list(EXCLUDED_ROUTES),
...
def render_all(
    ...
    icons: dict[str, Any],
    full_rulesets: list[CompiledRuleset],
    profile_rulesets: list[CompiledRuleset] | None = None,
) -> dict[str, Any]:
    dist = root / "dist"
    profile_rulesets = profile_rulesets if profile_rulesets is not None else full_rulesets
    render_policies = dict(policies)
    render_policies["_compiled_rulesets"] = profile_rulesets
    collections = {
        "full": (RULES_FULL_DIR, full_rulesets),
        "profile": (RULES_PROFILE_DIR, profile_rulesets),
    }
    skipped: dict[str, dict[str, dict[str, int]]] = {
        mode: {target: {} for target in TARGETS} for mode in collections
    }
    for mode, (directory, rulesets) in collections.items():
        ...
        for target in TARGETS:
            for ruleset in rulesets:
                ...
                path = dist / target / directory / rule_filename(target, ruleset.id)
                expected[target].add(path)
                write_if_changed(path, content)
                if skipped_count:
                    skipped[mode][target][ruleset.id] = skipped_count

            rules_dir = dist / target / directory
            if rules_dir.exists():
    ...
    # PR #3 replaces the ambiguous legacy directory with explicit full/profile
    # variants. Remove only known generated rule files; never delete user files.
    for target in TARGETS:
        legacy = dist / target / "rules"
